...
class DB(object):
    def __init__(
        ...
        self.info = info.get("info", {}).get("0.0_DB", {})
        self.schema = schema
        self.type = "DB"
        self.level = str(info.get("level", 0))
        if self.level == "0":
            self.level = "0.0"
        ...
        self.site = self.to_db_site_list()
        self.site_info = self.to_db_site_info_list()
    ...
    def to_db_level(self, level: Union[int, float, str] = 0) -> str:
        ...
        for db in self.db_sites_rule:
            if re.match(r"^(.*SITE)(.*)?$", db, re.IGNORECASE) and level > 3:
    ...
    def to_sum_info(self):
        ...
        if float(self.level) > 0 and db_id:
            sum_db_info_dct = {
